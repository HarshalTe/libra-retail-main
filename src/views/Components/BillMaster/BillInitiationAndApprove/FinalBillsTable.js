...
import FinallBillApproveAction from "./FinallBillApproveAction";
import ViewFinalBill from "./ViewFinalBill";
import ApproveFinalBill from "./ApproveFinalBill";
    ...
    >
      ...
      {numSelected > 0 ? (
        <div className="d-flex">
          {/* {bill_initiation_approval_page.update_status == "1" ? (
            <ApproveFinalBill data={selected} />
          ) : (
            ""
          )} */}
          {bill_initiation_approval_page.update_status == "1" ? (
            <FinallBillApproveAction data={selected} />
          ) : (
            ""
          )}
        </div>
...
function FinalBillsTable(props) {
  //*
  const bill_initiation_approval_page = props?.login?.login?.user?.rights.find(
...
const [selectionModel, setSelectionModel] = React.useState([]); 
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props?.finalBills?.finalBills?.data?.map(
        (row) => row
      );
  ...
  const emptyRows =
    page > 0
      ? Math.max(
          0,
          (1 + page) * rowsPerPage - props?.finalBills?.finalBills?.data?.length
        )
      : 0;
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <EnhancedTableToolbar
        ...
        bill_initiation_approval_page={bill_initiation_approval_page}
      />
...
export default connect(mapStateToProps, mapDispatchToProps)(FinalBillsTable);
