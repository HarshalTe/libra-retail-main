...
import ViewReopenBill from "./ViewReopenBill";
import CancelledBill from "./CancelledBill";
...
function ReopenBillTable(props) {
  ...
  const columns2 = [
    ...
    renderCell: (row) => {
      return  <ViewReopenBill data={row.row} />
    },
  },
  ];
    ...
    if (event.target.checked) {
      const newSelecteds = props?.finalBills?.finalBills?.data
        ?.filter(
          (row) =>
            row.is_approved == 1 &&
            row.is_reopen_closed == 1 &&
            row.is_reopen_closed_processed == 0
        )
        .map((row) => row);
...
export default connect(mapStateToProps, mapDispatchToProps)(ReopenBillTable);
