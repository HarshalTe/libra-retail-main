...
import CancelledBill2 from "./CancelledBill2";
...
function CancelledBillsTables(props) {
  ...
  : props.finalBills.finalBills.data?.length > 0
    ...
    ?  props?.finalBills?.finalBills?.data?.filter((pur) => {
      ...
      :props.finalBills.finalBills.data?.filter((pur,i)=>{
        return(
          pur?.is_reopen_closed_processed == 1
          )
      })
      ...
      const newSelecteds = props?.finalBills?.finalBills?.data
        ?.filter(
          (row) =>
            ...
            row.is_reopen_closed_processed == 1
        )
        .map((row) => row);
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Table id="table-CancelledBillsTables" sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
