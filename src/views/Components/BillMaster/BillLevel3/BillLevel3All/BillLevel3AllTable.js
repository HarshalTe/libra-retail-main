...
import ViewBillLevel3 from "./ViewBillLevel3";
...
function BillLevel3AllTable(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props?.finalBills?.finalBills?.data
        ?.filter((row) => row.is_approved == 1 && row.is_processed == 0)
        .map((row) => row);
      ...
      ? Math.max(
          ...
          (1 + page) * rowsPerPage -
            props?.finalBills?.finalBills?.data?.filter(
              (row) => row.is_approved == 1 && row.is_processed == 0
            ).length
        )
      : 0;
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Table id="table-BillLevel3AllTable" sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
...
export default connect(mapStateToProps, mapDispatchToProps)(BillLevel3AllTable);
