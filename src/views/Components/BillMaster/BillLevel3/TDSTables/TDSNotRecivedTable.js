...
import ViewTDSBill from "./ViewTDSBill";
import TDSReceived from "./TDSRecived";
...
function TDSNotRecivedTable(props) {
  ...
  const columns2 = [
    ...
    renderCell: (row) => {
      return  <ViewTDSBill data={row.row} />
  ...
  : props.finalBills.finalBills.data?.length > 0
    ...
    ? props.finalBills?.finalBills?.data?.filter((pur,i)=>{
      ...
      console.log("hhhhh",DateFormat({ data: item.created_at}),"h",DateFormat({ data: props.DateData?.StartDate }),"h",DateFormat({ data: item.created_at}),"h" ,DateFormat({ data: props.DateData?.EndDate }))
        return (
                (props.DateData?.StartDate != null && props.DateData?.EndDate != null 
            ? DateFormat({ data: item.created_at}) >= DateFormat({ data: props.DateData?.StartDate }) && 
              DateFormat({ data: item.created_at}) <= DateFormat({ data: props.DateData?.EndDate })
            : item)
                    );
                  })
                  : [];
  console.log(rows, "hhhhhh",props,props.DateData?.StartDate)
  const fetchData = () => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props?.finalBills?.finalBills?.data
        ?.filter((row) => row.is_approved == 1 && row.is_tds == 0)
        .map((row) => row);
      ...
      ? Math.max(
          ...
          (1 + page) * rowsPerPage -
            props?.finalBills?.finalBills?.data?.filter(
              (row) => row.is_approved == 1 && row.is_tds == 0
            ).length
        )
      : 0;
      const rowsCopy2 = [...rows2];
      const reversedRows2 = rowsCopy2.reverse();
      const rowsCopy = [...rows];
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <div style={{ height: 650, width: '100%' }}>
        <DataGrid
          rows={filter==true ? reversedRows2 :reversedRows}
          columns={columns2}
          pageSize={10}
      ...
      <Table id="table-TDSNotRecivedTable" sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
...
export default connect(mapStateToProps, mapDispatchToProps)(TDSNotRecivedTable);
