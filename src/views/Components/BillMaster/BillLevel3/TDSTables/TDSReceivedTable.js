...
function TDSReceivedTable(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props?.finalBills?.finalBills?.data
        ?.filter((row) => row.is_approved == 1 && row.is_tds == 1)
        .map((row) => row);
      ...
      ? Math.max(
          ...
          (1 + page) * rowsPerPage -
            props?.finalBills?.finalBills?.data?.filter(
              (row) => row.is_approved == 1 && row.is_tds == 1
            ).length
        )
      : 0;
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Table id="table-TDSReceivedTable" sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
...
export default connect(mapStateToProps, mapDispatchToProps)(TDSReceivedTable);
