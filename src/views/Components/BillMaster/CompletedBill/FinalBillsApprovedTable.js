...
import ReopenBillButton from "./ReopenBillButton";
import PaidBill from "./PaidBill";
import DigitalSign from "./DigitalSign";
import EditBill from "./EditBill";
import ReopenFinalBill from "./ReopenFinalBill";
...
import ReopenButton from "views/Components/CaseMaster/CompletedCases/ReopenButton";
...
function FinalBillsApprovedTable(props) {
  ...
  const [bill_no, setBill_no] = React.useState("");
...
: props.finalBills.finalBills.data?.length > 0
  ...
  ?  props?.finalBills?.finalBills?.data?.filter((pur) => {
    ...
    return(
      (bill_no
        ? pur?.bill_no
            ?.toLowerCase().includes(bill_no.trim().toLowerCase())
        : pur) &&
      (year
  ...
  : props.finalBills.finalBills.data?.filter((pur,i)=>{
    return(
      pur?.is_approved == 1 && pur?.is_reopen_by_admin == 0
      )
  })
  : [];
...
const columns2 = [
  ...
  renderCell: (row) => {
    ...
    <ReopenFinalBill  data={row.row}/>
    <ViewFinalBill data={row.row} />
    <PrintFinalBill data={row.row} />
  ...
  return (<>
          ...
          >
            {(formProps) => {
              setBill_no(formProps.values.bill_no);
