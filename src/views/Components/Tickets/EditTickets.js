...
//* Actions
import { ticketsEditData } from "../../../Redux/Creators/TicketsCreators";
...
function EditQuries(props) {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    props.ticketsEditData(data,token);
    setSubmitting(true);
    setModal(false);
  };
                ...
                <Row>
                  <Col md={6} className="pb-4">
                    <>
                      <Autocomplete
                        id="case_id"
                        options={propertiesProps.options}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ticketsEditData: (data,token) => dispatch(ticketsEditData(data,token)),
  };
};
